...
describe('create project API', function () {
	...
	before(() => {
		return Promise.all([
			...
			new APIToken().saveAsync().spread((token) => {
	...
	describe('POST to /API/build/new%20project', () => {
		let response;
		before(function () {
	...
	describe('unathenticated POST to /API/build/new%20project', () => {
		it('returns [unathenticated]', () => {
