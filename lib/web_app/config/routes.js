...
var ControllerBase = require('../controllers/controller_base');

module.exports = function (server) {

  return glob(path.resolve(__dirname, '../controllers/**/*.js'))
    .then(function (files) {
      logger.debug({
        files: files
      ...
      files.forEach(function (file) {
        ...
        if ((!Controller) || (!_.isFunction(Controller)) || (Controller === ControllerBase)) {
          return;
        }
      ...
      server.route([{
        ...
        config: {
          handler: {
            directory: {
              path: path.resolve(__dirname, '../assets/src/fonts'),
              lookupCompressed: true
            }
          },
          cache: {
