...
function rebalanceExecutor(channel, queue) {
      ...
      .then(function (channel) {
        return channel.assertExchange('executor_commands', 'topic')
          .then(function (response) {
            ...
            /* jshint camelcase:false */
            return channel.publish(response.exchange, 'command:' + channel.peer_host + ':' + queue, new Buffer(JSON.stringify({
              task: 'reaquire',
              consumerTag: channel.consumer_tag
            })));
          });
      }).then(function () {
  ...
  then(function (queues) {
    ...
    return setScaleMinAndMax(queues.length, messageCount)
      .then(function () {
          ...
          if (queuesWithTooManyConsumers.length > 0) {
            ...
            return getConsumersForQueues(queuesWithTooManyConsumers)
              .then(function (consumers) {
                ...
                return rebalanceExecutor(selectedChannel, selectedConsumer.queue.name).then(function () {
                  return false;
                });
              });
          } else {
            ...
            return autoscaling.executePolicyAsync(scaleUpParams).the(function () {
              return true;
            });
          }
        ...
        if (!done) {
          return BBPromise.delay(5000).then(function () {
