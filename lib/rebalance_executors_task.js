...
function setScaleMinAndMax(queueCount, messages) {
  var max = Math.floor((messages / 1000) + (queueCount * 2));
  logger.info({
    max: max
  }, 'new max');
  var autoScaleConfig = {
    MinSize: queueCount,
    MaxSize: Math.max(20, max),
    ...
    return setScaleMinAndMax(queues.length, messageCount)
      .then(function () {
        ...
        var queuesWithTooFewConsumers = _.filter(queues, function (queue) {
          ...
          return queue.consumers > 0 && ((queue.consumers / queue.messages_ready) < 0.001);
        });
        logger.info({
        ...
        var queuesWithTooManyConsumers = _.filter(queues, function (queue) {
          /* jshint camelcase:false */
          return queue.consumers > 2 && ((queue.consumers / queue.messages_ready) > 0.001);
        });
        logger.info({
        ...
        if (queuesThatHaveNoConsumers.length > 0 || queuesWithTooFewConsumers.length > 0) {
          ...
          if (queuesWithTooManyConsumers.length > 0) {
            ...
            return autoscaling.executePolicyAsync(params);
          }
