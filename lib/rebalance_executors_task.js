    ...
    return setScaleMinAndMax(queues.length, messageCount)
      .then(function () {
        ...
        var queuesWithTooFewConsumers = _.filter(queues, function (queue) {
          /* jshint camelcase:false */
          return queue.consumers > 4 && queue.consumers < 40 && ((queue.consumers / queue.messages_ready) < 0.001);
        });
        logger.info({
        ...
        var queuesWithTooManyConsumers = _.filter(queues, function (queue) {
          /* jshint camelcase:false */
          return queue.consumers > 40 || (queue.consumers > 4 && ((queue.consumers / queue.messages_ready) > 0.001));
        });
        logger.info({
