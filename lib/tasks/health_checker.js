...
class HealthChecker {
  ...
  markHealth(instance, connections) {
      ...
      .then(() => {
        let failedInstances = healthCheck.instances.filter((instance) => {
          return instance.currentState === 'unhealthy' &&
            new Moment(instance.failedSince).isBefore(new Moment().subtract(15, 'minutes'));
        });
        this._logger.info({
  ...
  check() {
    return this.loadEC2Details()
      .then((instances) => {
        return this.loadRabbitConnections()
          .then((connections) => {
            this._logger.info({
              connections: connections.length
            }, 'connections');
            instances.forEach((instance) => {
              this.markHealth(instance, connections);
            });
