...
class HealthChecker {
  ...
  markHealth(instance, connections, queues) {
    if (instance.launchTime.isAfter(new Moment().subtract(15, 'minutes'))) {
    ...
    } else if (includes(connections, instance.privateIpAddress)) {
      this._logger.info({
        instance: instance.id
      }, 'instance connected');
      let connectedQueues = filter(queues, (queue) => {
        let matches = new RegExp(`executor-[abcdefghij]-${instance.id}`).test(queue);
        return matches;
      });
      this._logger.info({
        instance: instance.id,
      ...
      if (connectedQueues.length > 9) {
        this._logger.info({
          instance: instance.id
        }, 'executor VM has 10 executors connected');
        instance.healthy = true;
      } else {
        this._logger.info({
          instance: instance.id
        }, 'executor VM has less than 10 executors connected');
