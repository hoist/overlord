...
class ExecutorScaler extends Rebalancer {
  constructor() {
    super();
    ...
    return Promise.resolve()
      .then(() => {
        return currentReading.queues.map((queue) => {
          ...
          if (queueStats1 === null && queueStats2 === null && queueStats3 === null) {
          ...
          } else if (queueStats1 === null || queueStats2 === null || queueStats3 === null) {
            this._logger.info({
              queue: queue.name
            ...
            if (queue.messagesReady === queueStats1.messagesReady === queueStats2.messagesReady === queueStats3.messagesReady === 0) {
              ...
              if (averageDifference > 0 && queue.consumers < 20) {
                executorChange += 1;
              } else if (averageDifference < 0 && queue.consumers > 2) {
                executorChange -= 1;
              } else if (averageDifference === 0 && queue.messagesReady > 1000 && queue.consumers < 20) {
                executorChange += 1;
              }
              this._logger.info({
                queue: queue.name,
      ...
      .then(() => {
        ...
        return this.generateDifference(firstReading, secondReading, thirdReading, currentReading)
          .then((difference) => {
            ...
            let newExecutorVMCount = Math.ceil(newTotalExecutors / 2) + 1;
            this._logger.info({
              executorChange,
              newTotalExecutors,
              newExecutorVMCount
            ...
            let configUpdate = {
              MinSize: currentReading.queues.length + 1,
              MaxSize: (currentReading.queues.length * 10) + 1,
