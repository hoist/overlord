...
class Rebalancer {
  ...
  getExecutorCount(queue) {
    ...
    return Math.max(2, Math.min(20, Math.floor(queue.messages_ready / 200)));
  };
  ...
  getConsumerStats(queue) {
    ...
    return request.get(url.format(rabbitUrl), {
      ...
      .then((response) => {
        ...
        stats.expectedConsumers = Math.max(2, Math.min(20, Math.floor(response.messages_ready / 200)));
